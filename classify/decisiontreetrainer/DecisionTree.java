...
public class DecisionTree extends Predictor {
    ...
    private void preComputeAndCacheMeans(int totalFeatures, List<Instance> instances) {
        ...
        for (Instance instance : instances)
            for (Integer feature : instance.getFeatureVector().getFeatureVectorKeys()) {
                precomputedMeans.put(feature, precomputedMeans.get(feature) + instance.getFeatureVector().get(feature));
            }
        for (int index : precomputedMeans.keySet())
            precomputedMeans.put(index, precomputedMeans.get(index) / instances.size());
    }

    public Node buildDecisionTree(List<Instance> instancesInCurrentTree, int maxDepth) throws CloneNotSupportedException {
        // no instances to work on?
        if (instancesInCurrentTree.size() == 0) {
            System.out.println("Ran out of instances");
            Label majority = getMajorityLabel(trainingInstances);
            System.out.println("label -- " + majority);
            System.out.println("=======================================================================================");
        ...
        if(leftSubTree.size() == instancesInCurrentTree.size() || rightSubtree.size() == instancesInCurrentTree.size()) {
            System.out.println("=======================================================================================");
    ...
    private void removeFeatureFromFeatureVector(int featureIndex , List<Instance> instances) {
        for(Instance instance : instances) {
            FeatureVector fv = instance.getFeatureVector();
            fv.featureVector.remove(featureIndex);
        }
    }
    ...
    private void divideFeatureVectorsBasedOnMean
            (List<Instance> instances, List<Instance> leftSubTree,
             ...
             double mean,
             int featureIndex) throws CloneNotSupportedException
    {
